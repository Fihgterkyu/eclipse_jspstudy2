...
<%
  ...
  int count=0;//총레코드수
  int number=0;//beginPerPage->페이지별로 시작하는 맨처음에
                       //                        나오는 게시물번호
  ...
  if(count > 0){//한개라도 레코드수가 있다면   endRow(X)
	  articleList=dbPro.getArticles(startRow, pageSize);//10개
...
<table border="1" width="700" cellpadding="0" cellspacing="0" align="center"> 
   ...
   <%
       for(int i=0;i<articleList.size();i++){
    	  //                                        ariticleList.elementAt(i)
         BoardDTO article=(BoardDTO)articleList.get(i);
   %> 
   <tr height="30"><!-- 하나씩 감소시키면서 출력하는 게시물번호 -->
